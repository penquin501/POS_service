                  ...
                  if (res) {
                    if (
                      ...
                      let updateBilling =
                        ...
                        data[0].billing_no +
                        "'";
                      connection.query(updateBilling, function(
                        err,
                        dataBilling
                      ) {});
                      ...
                      for (q = 0; q < listStatus.length; q++) {
                        ...
                        var dateTimeString = moment(new Date()).format(
                          "YYYY-MM-DD HH:mm:ss",
                          true
                        );
                        updateServices
                          .updateStatusReceiverInfo(
                            tracking,
                            status,
                            dateTimeString
                          )
...
function pushCaptureData() {
  ...
  connection.query(sqlParcelCapture, function(err, result) {
    ...
    var sqlCapture =
      "SELECT consignmentno,rawdata FROM parcel_temp_capture WHERE recorddate = '" +
      dateTimeString +
      "' AND consignmentno NOT IN (" +
      listBarcode.replace(/,$/, "") +
      ") LIMIT 1";
...
function insertKeyInData() {
  var sqlParcelKeyIn =
    "SELECT br.tracking FROM billing_item bi " +
  ...
  connection.query(sqlParcelKeyIn, function(err, result) {
    if (result.length !== 0) {
      for (i = 0; i < result.length; i++) {
        var sqlKeyIn =
          "SELECT consignmentno,rawdata,recordtimestamp FROM parcel_keyin_data WHERE consignmentno='" +
          result[i].tracking +
...
function pushKeyInData() {
  var max_record = "";
  var sqlParcelKeyIn =
    "SELECT MAX(record_at) as max_record_at FROM record_keyin WHERE Date(record_at)=CURRENT_DATE()";
  connection.query(sqlParcelKeyIn, function(err, result) {
    if (!result) {
      max_record = "''";
    } else {
      var max_record_datetime = moment(result[0].max_record_at).format(
        "YYYY-MM-DD HH:mm:ss",
        true
      );
    ...
    var sqlKeyIn =
      "SELECT consignmentno,rawdata,recordtimestamp FROM parcel_keyin_data where recorddate=CURRENT_DATE and recordtimestamp>'" +
      max_record +
      "'";
    billing_connection.query(sqlKeyIn, function(err, result2) {
      if (result2.length !== 0) {
        for (j = 0; j < result2.length; j++) {
          var dateTimeString = moment(result2[j].recordtimestamp).format(
            "YYYY-MM-DD HH:mm:ss",
            true
          );
          var sqlKeyInRecord =
            "INSERT INTO record_keyin(tracking, record_at) VALUES ('" +
            result2[j].consignmentno +
            "','" +
            dateTimeString +
            "')";
...
function pushReceiverTempToReceiverInfo() {
  var sqlReceiverInfo =
    "SELECT tracking,parcel_type,zipcode FROM billing_receiver_info where receiver_name is null AND remark ='QUICKLINK'";
  connection.query(sqlReceiverInfo, function(err, result) {
    if (result.length !== 0) {
      for (i = 0; i < result.length; i++) {
        ...
        var sqlReceiverTempInfo =
          "SELECT * FROM billing_receiver_info_temp where tracking='" +
          tracking_info +
          "'";
        connection.query(sqlReceiverTempInfo, function(err, result2) {
          if (
            result2.length !== 0 &&
            parcel_type_info == result2[0].parcel_type &&
            zipcode_info == result2[0].zipcode
          ) {
            var sqlUpdateReceiver =
              "UPDATE billing_receiver_info SET receiver_name='" +
              result2[0].receiver_name +
              "',phone='" +
              result2[0].phone +
              "',receiver_address='" +
              result2[0].receiver_address +
              "',district_id='" +
              result2[0].district_id +
              "',district_name='" +
              result2[0].district_name +
              "'," +
              "amphur_id='" +
              result2[0].amphur_id +
              "',amphur_name='" +
              result2[0].amphur_name +
              "',province_id='" +
              result2[0].province_id +
              "',province_name='" +
              result2[0].province_name +
              "',remark='KEYIN' where tracking='" +
              result2[0].tracking +
              "'";
